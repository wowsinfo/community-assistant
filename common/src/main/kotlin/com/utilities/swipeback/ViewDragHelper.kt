...
class ViewDragHelper private constructor(context: Context, forParent: ViewGroup?, cb: Callback?) {
    ...
    private var mInitialMotionY: FloatArray = FloatArray(0)
    private var mLastMotionX: FloatArray = FloatArray(0)
    private var mLastMotionY: FloatArray = FloatArray(0)
    private var mInitialEdgeTouched: IntArray = IntArray(0)
    ...
    fun setSensitivity(context: Context?, sensitivity: Float) {
        val s = max(0.0, min(1.0, sensitivity.toDouble())).toFloat()
    ...
    private fun getEdgeTouched(x: Int, y: Int): Int {
        ...
        if (x < mParentView.left + edgeSize) result = EDGE_LEFT
        if (y < mParentView.top + edgeSize) result = EDGE_TOP
        if (x > mParentView.right - edgeSize) result = EDGE_RIGHT
        if (y > mParentView.bottom - edgeSize) result = EDGE_BOTTOM
